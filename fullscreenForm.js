...
;( function( window ) {
	...
	FForm.prototype._addControls = function() {
		...
		this.ctrlContinue = createElement( 'button', { cName : 'fs-continue fas fa-chevron-down', inner : '', appendTo : this.ctrls } );
...
setTimeout( function(){ document.querySelector('.fs-current > input').focus(); } , 500 );
		...
		this.fields.forEach( function( fld ) {
			if( fld.hasAttribute( 'data-input-trigger' ) ) {
				var input = fld.querySelector( 'input[type="radio"]' ) || /*fld.querySelector( '.cs-select-skin' ) ||*/ fld.querySelector( 'select' ); // assuming only radio and select elements (TODO: exclude multiple selects)
				if( !input ) return;
